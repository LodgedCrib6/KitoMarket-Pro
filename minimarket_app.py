...
class MinimarketApp(ctk.CTk):
    def __init__(self):
        ...
        self.iconbitmap("KitoLogo.ico")  # ícono en la ventana
    ...
    def _escalar_fuentes(self, w, h):
        # Base de referencia: 1100x850
        escala = min(w / 1100, h / 850)
        escala = max(escala, 0.45)  # mínimo para no quedar ilegible

        t_precio  = max(int(120 * escala), 40)
        t_nombre  = max(int(28  * escala), 14)
        t_guia    = max(int(16  * escala), 10)
        t_scan    = max(int(36  * escala), 16)
        t_semaf   = max(int(18  * escala), 10)
        t_hist    = max(int(12  * escala), 8)
        ancho_scan = max(int(400 * escala), 180)
        alto_scan  = max(int(65  * escala), 30)
